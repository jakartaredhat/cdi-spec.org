...
To make it short the goal with CDI lite is to make the spec cloud native. This will be done by identifying a features' subset that can be implemented with a build time only approach (no more runtime proxies and other “magic” requirements) .

The idea is to keep the CDI programming model everyone loves but allow implementation to produce lighter and faster binaries.

One issue we raised in the last post is that this build-time approach is not compatible with CDI portable extension. So, with current CDI features we can only imagine CDI Lite without extension mechanism. Knowing the great popularity of portable extensions, it was a capability we needed to revisit in our initial proposal.

It’s the reason why we worked to propose a new extension mechanism for CDI Lite. Today we are sharing this proposal with you as it is still a work in progress but seems ready to be a strong starting point to design an extension mechanism for CDI lite.
...
These methods can declare an arbitrary number of parameters, all of which are supplied by the container. There’s a set of predefined types of parameters for each phase, and all implementations would have to support that. We’re also thinking of designing an SPI that would allow anyone to contribute support for other types of parameters.
A class can declare multiple extensions, in which case they are all invoked on a single instance of the class. If you need to control order of extension invocations, there’s an annotation *`@ExtensionPriority`* just for that.
...
As mentioned before, we are very keen on hearing your feedback. Please file issues in the *TODO* GitHub repository, or send an email to the *TODO* mailing list. Let’s work together on making these new Build Compatible Extensions a reality!
